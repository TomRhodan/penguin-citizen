...
// Empirically captured SC attribute → UI-label mappings generated by the
// `sc-watch` tool. Source of truth lives under src/data/; regenerate by
// running sc-watch with --log LabelLogs/<file>.md and copying the resulting
// .json over the canonical file (or symlinking it).
import SC_ATTRIBUTE_LABELS from '../../data/sc-attribute-labels.json';
...
export const DEFAULT_SETTINGS = {
  ...
  // SC USER.cfg encoding (verified via starcitizen-lug/knowledge-base
  // wiki/Tips-and-Tricks.md): r.graphicsRenderer = 0 -> DX11, = 1 -> Vulkan.
  // Default = 1 to match SC 4.5+'s in-game default (Vulkan); generateUserCfg
  // only emits the line when the user actively selects DX11.
  'r.graphicsRenderer': { value: 1, label: 'Graphics Renderer', min: 0, max: 1, step: 1, category: 'essential', labels: ['DX11', 'Vulkan'],
    target: 'usercfg',
    desc: 'Graphics API: Vulkan (recommended) or DX11',
    help: 'Selects the graphics API. Vulkan is the default since 4.5 and pre-builds shaders to reduce stuttering. DX11 is a legacy fallback that is being deprecated; only switch if Vulkan causes crashes on your hardware.' },
  ...
  // LUG knowledge-base (Tips-and-Tricks.md) documents r_displayinfo with values
  // [0, 1, 2, 3]. Level 4 was an unverified claim — clamping to the documented
  // range until verified empirically.
  r_DisplayInfo: { value: 0, label: 'Debug HUD', min: 0, max: 3, step: 1, category: 'essential',
    target: 'usercfg',
    desc: 'Performance debug overlay (0=off, 1-3 detail)',
    help: 'Shows real-time performance metrics on screen. Level 1 shows basic FPS/details, level 2 adds the frame-time chart, level 3 shows the most detail. Useful for troubleshooting; disable for normal play.' },
  ...
  // SC's graphics-quality scale is 1-Low, 2-Medium, 3-High, 4-Very High (no
  // "Ultra"; verified against Chadarius/sc-config user.cfg and the in-game UI).
  // Max stays at 4 — sliders are also clamped on load via convertAttrValue.
  sys_spec: { value: 3, label: 'Overall Quality', min: 1, max: 4, step: 1, category: 'quality',
    ...
    help: 'Sets the global graphics quality preset. 1=Low, 2=Medium, 3=High, 4=Very High. Star Citizen UI labels this as "Overall Quality Preset". Adjust individual settings below to fine-tune after choosing a base preset.' },
  ...
  SysSpec_ObjectViewDistance: { value: 3, label: 'Object View Distance', min: 1, max: 4, step: 1, category: 'quality',
  ...
  sys_spec_Texture: { value: 3, label: 'Texture Quality', min: 1, max: 4, step: 1, category: 'quality',
  ...
  SysSpec_TextureDetail: { value: 3, label: 'Detail Textures', min: 1, max: 4, step: 1, category: 'quality',
  ...
  SysSpec_TextureGround: { value: 3, label: 'Ground Textures', min: 1, max: 4, step: 1, category: 'quality',
  ...
  SysSpec_TextureFiltering: { value: 3, label: 'Texture Filtering', min: 1, max: 4, step: 1, category: 'quality',
  ...
  sys_spec_Shadows: { value: 3, label: 'Shadow Maps', min: 1, max: 4, step: 1, category: 'quality',
  ...
  SysSpec_ShadowScreenSpace: { value: 3, label: 'Screen Space Shadows', min: 1, max: 4, step: 1, category: 'quality',
  ...
  SysSpec_PlanetVolumetricClouds: { value: 4, label: 'Planet Volumetric Clouds', min: 1, max: 4, step: 1, category: 'quality',
  ...
  SysSpec_GasCloud: { value: 4, label: 'Gas Clouds', min: 1, max: 4, step: 1, category: 'quality',
  ...
  SysSpec_Fog: { value: 4, label: 'Fog', min: 1, max: 4, step: 1, category: 'quality',
  ...
  sys_spec_Water: { value: 3, label: 'Water Simulation', min: 1, max: 4, step: 1, category: 'quality',
  ...
  SysSpec_WaterCaustics: { value: 4, label: 'Water Caustics', min: 1, max: 4, step: 1, category: 'quality',
  ...
  sys_spec_Shading: { value: 3, label: 'Shader Quality', min: 1, max: 4, step: 1, category: 'quality',
  ...
  SysSpec_PostProcessingAttr: { value: 3, label: 'Post Effects', min: 1, max: 4, step: 1, category: 'quality',
  ...
  SysSpec_VideoComms: { value: 3, label: 'Video Comms', min: 1, max: 4, step: 1, category: 'quality',
  ...
  sys_spec_GameEffects: { value: 3, label: 'Game Effects (engine)', min: 1, max: 4, step: 1, category: 'quality',
  ...
  sys_spec_Physics: { value: 3, label: 'Physics (engine)', min: 1, max: 4, step: 1, category: 'quality',
    target: 'usercfg', overrideWarning: true,
...
/**
 * Apply empirical SC UI labels from sc-attribute-labels.json over the hardcoded
 * labels above. Only `setting.label` is mutated; min/max/category/help are
 * untouched. Settings not present in the JSON keep their hardcoded label.
 * Settings present in the JSON but not in DEFAULT_SETTINGS are ignored (we
 * can't expose them in the UI without min/max/type metadata).
 *
 * Re-generating the JSON: run `sc-watch` with `--log LabelLogs/<file>.md` (sc-watch
 * writes the `.json` next to the `.md` on Ctrl+C); copy the resulting JSON over
 * `src/data/sc-attribute-labels.json` to publish the updated labels.
 */
(function applyScLabelOverrides() {
  const attrLabels = SC_ATTRIBUTE_LABELS?.files?.['attributes.xml'] || {};
  const cfgLabels = SC_ATTRIBUTE_LABELS?.files?.['USER.cfg'] || {};
  for (const [key, setting] of Object.entries(DEFAULT_SETTINGS)) {
    if (setting.target === 'attributes' && setting.attrName) {
      const sc = attrLabels[setting.attrName];
      if (sc?.label) setting.label = sc.label;
    } else if (setting.target === 'usercfg') {
      const sc = cfgLabels[key];
      if (sc?.label) setting.label = sc.label;
    }
  }
})();
...
/** Display labels for graphics quality levels (1-4). Index 0 unused; SC's UI exposes Low → Very High (no "Ultra"; verified against Chadarius/sc-config user.cfg). */
...
export function getSettingLabels(key) {
  const map = {
    ...
    'r.graphicsRenderer': () => [t('environments:cfg.renderer.dx11'), t('environments:cfg.renderer.vulkan')],
 ...
 *
 * Quality-keyed values (SysSpec / sys_spec_*) are clamped to the SC-valid range
 * 1-4. Older app versions allowed value 5 ("Ultra") — historical attributes.xml
 * files may still contain that. Clamping here prevents UI glitches (out-of-
 * range slider, label lookup with undefined index) and self-heals on next Apply.
 */
export function convertAttrValue(key, attrStringValue) {
  ...
  if (isNaN(num)) return attrStringValue;
  if (QUALITY_KEYS.has(key)) return Math.min(Math.max(num, 1), 4);
  return num;
}
  ...
  for (const line of lines) {
    ...
    if (trimmed && !trimmed.startsWith(';') && !trimmed.startsWith('#')) {
      ...
      if (match) {
        ...
        // Clamp legacy quality values (older app versions allowed 5 "Ultra";
        // SC only supports 1-4). Same logic as in convertAttrValue.
        if (QUALITY_KEYS.has(key) && typeof value === 'number') {
